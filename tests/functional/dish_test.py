...
def test_dish_create(ids):
    ...
    assert response.json()['price'] == '{0:.2f}'.format(payload['price'])
...
def test_update_dish(ids):
    ...
    response = client.patch(f"api/v1/menus/{ids['menu_id']}/submenus/{ids['submenu_id']}/dishes/{ids['dish_id']}", json=payload)
    assert response.status_code == 200
